...
func Key(projectPath string, pipelineID int) string {
	return fmt.Sprintf("%s#%d", projectPath, pipelineID)
}
...
type Entry struct {
	ProjectPath string
	PipelineID  int
	MRIID       string // the MR this pipeline belongs to, if known (display only)

	// Last observed state, updated on each successful fetch.
	Status    string         // pipeline status, "" until first fetch
	JobStatus map[int]string // jobID -> status
	WebURL    string
...
func activeStatus(s string) bool {
	switch s {
	case "running", "pending", "created", "scheduled", "waiting_for_resource", "preparing":
		return true
	default:
		return false
	}
...
// (returns false).
func (r *Registry) Toggle(projectPath string, pipelineID int, mrIID string) bool {
	r.mu.Lock()
	defer r.mu.Unlock()
	k := Key(projectPath, pipelineID)
...
func (r *Registry) Remove(projectPath string, pipelineID int) {
	r.mu.Lock()
	defer r.mu.Unlock()
	delete(r.entries, Key(projectPath, pipelineID))
}

// Has reports whether a pipeline is being watched.
func (r *Registry) Has(projectPath string, pipelineID int) bool {
	r.mu.Lock()
	defer r.mu.Unlock()
	_, ok := r.entries[Key(projectPath, pipelineID)]
	return ok
}
...
// ActiveTargets returns the (projectPath, pipelineID) of entries still worth
...
func (r *Registry) Update(projectPath string, p *gitlab.Pipeline) Change {
	r.mu.Lock()
	defer r.mu.Unlock()

	e, ok := r.entries[Key(projectPath, p.ID)]
	if !ok {
		return Change{}
	}
	...
	e.Status = p.Status
	e.WebURL = p.WebURL
	e.JobStatus = make(map[int]string, len(p.Jobs))
	...
	for _, j := range p.Jobs {
		...
		if !activeStatus(j.Status) && finished == "" {
			finished = j.Name
		}
	}
	if finished != "" {
